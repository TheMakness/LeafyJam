...
void CalculateMainLight_float(float3 WorldPos, out half3 Direction, out float3 Color, out half DistanceAtten, 
...
void AddAdditionalLights_float(float Smoothness, float3 WorldPosition, float3 WorldNormal,
	float3 WorldView, float MainDiffuse, float MainSpecular, float3 MainColor, out float Diffuse, out float Specular, out float3 Color)
{
	Diffuse = MainDiffuse;
